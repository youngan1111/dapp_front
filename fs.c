...
itrunc(struct inode *ip)
{
    ...
    // double indirect인 경우 release 해주기 위를 중첩으로 2번 해주기
    ...
    // tripple indirect block 경우
    if (ip->addrs[NDIRECT + 2])
    {
        bp0 = bread(ip->dev, ip->addrs[NDIRECT + 2]);
        a0 = (uint *)bp0->data;
        for (i = 0; i < NINDIRECT; i++)
        {
            if (a0[i])
