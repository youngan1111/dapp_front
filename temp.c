...
int main(int argc, char *argv[])
{
    ...
    printf("nmeta %d (boot, super, log blocks %u inode blocks %u, bitmap blocks %u) blocks %d total %d %lu\n\nhello\n\n",
           nmeta, nlog, ninodeblocks, nbitmap, nblocks, FSSIZE, sizeof(struct dinode));
...
void iappend(uint inum, void *xp, int n)
{
    ...
    while (n > 0)
    {
        ...
        { // double indirect
            ...
            rsect(xint(double_indirect[(fbn - NDIRECT - NINDIRECT) / NINDIRECT]), (char *)indirect);
            if (indirect[(fbn - NDIRECT - NINDIRECT) % NINDIRECT] == 0)
            {
                indirect[(fbn - NDIRECT - NINDIRECT) % NINDIRECT] = xint(freeblock++);
                wsect(xint(double_indirect[(fbn - NDIRECT - NINDIRECT) / NINDIRECT]), (char *)indirect);
            }
